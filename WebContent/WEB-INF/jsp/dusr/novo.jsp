...
function existe(id){
    $.get('existe?dusr.id=' + id, function(){
        alert('USUÁRIO: '+ id );        
    });
}
</script>
	...
	<form id="dusrForm" action="${pageContext.request.contextPath}/dusr"
		...
		<div class="formulario">
			...
			<input type="hidden" name="dusr.dusrdtnas"
				value="<fmt:formatDate pattern="dd/MM/yyyy HH:mm:ss" value="${dusr.dusrdtnas}" />" />
			...
			<input type="hidden" name="dusr.dusrFotoBext"
				value="${dusr.dusrFotoBext}" /> <input type="hidden"
				name="dusr.dusrAEprof" value="${dusr.dusrAEprof}" /> <input
				type="hidden" name="dusr.dusrConect01" value="${dusr.dusrConect01}" />
			<input type="hidden" name="dusr.dusrAEacervoB"
				value="${dusr.dusrAEacervoB}" /> <input type="hidden"
				name="dusr.dusrXAEacervoB" value="${dusr.dusrXAEacervoB}" /> <input
				type="hidden" name="dusr.dusrCcusto" value="${dusr.dusrCcusto}" />
			<input type="hidden" name="dusr.dusrCdepto"
				value="${dusr.dusrCdepto}" /> <input type="hidden"
				name="dusr.dusrMTAB" value="${dusr.dusrMTAB}" /> <input
				type="hidden" name="dusr.dusrMatTransfEst"
			...
			<div class="form-actions">
				...
				<a href="javascript:void(0);" onclick="existe(${dusr.id}); return false;">Existe</a>				
			</div>
		</div>
	</form>
	<script type="text/javascript">
